...
export default function Generator(props) {
  const {poison,muscles, goal,setPoison,setGoal,setMuscles, updateWorkout} = props
  ...
  return (
    <SectionWrapper id={'generate'} header={"generate your workout "} title={['It\'s','Huge','o\'clock']}>
        ...
        {Object.keys(WORKOUTS).map((type, typeIndex) => {
          return (
            <button onClick={() =>{
              setMuscles([ ])
              setPoison(type)
            }} className={'py-3 px-4 duration-200 border  border-solid rounded-md bg-slate-950 hover:border-blue-600' + (type === poison ? ' border-blue-600' : ' border-blue-400') }
            key={typeIndex} >
      ...
      <Header index={'03'} title={'Become Baki'} description={'Select your ultimate objective.'}/>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
          return (
            <button onClick={() =>{
              setGoal(scheme)
            }} className={'py-3 px-4 duration-200 border  border-solid rounded-md bg-slate-950 hover:border-blue-600' + (scheme === goal ? ' border-blue-600' : ' border-blue-400') }
            key={schemeIndex} >
